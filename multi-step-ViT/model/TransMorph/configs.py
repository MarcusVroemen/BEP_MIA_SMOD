...
'''
def get_3DTransMorph_config(img_size, args):
    '''
    ...
    config.patch_size = 4
    config.in_chans = 2
    config.embed_dim = args.embed_dim  # 96
    config.depths = tuple(args.depths)  # (2, 2, 4, 2)
    config.num_heads = tuple(args.num_heads)  # (4, 4, 8, 8)
    config.window_size = tuple(args.window_size)  # (5, 6, 7)
    config.mlp_ratio = 4
    ...
    config.img_size = img_size #(160, 192, 224)
    return config
