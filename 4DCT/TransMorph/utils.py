...
def TRE(x_lm, y_lm):
    """Average TRE of two arrars with 3D landmarks"""
    x_lm=x_lm.detach().cpu().numpy()[0]
    y_lm=y_lm.detach().cpu().numpy()[0]
    TREs=[]
    for i in range(len(x_lm)):
        ds=0
        for j in range(len(x_lm[i])):
            d=(x_lm[i,j]-y_lm[i,j])**2
            ds+=d
        TRE=ds**(1/2)
        TREs.append(TRE)
    return np.average(TREs)  